...
def _cifar10_eval_transform() -> transforms.Compose:
...
def _cifar10_download_needed(data_dir: str) -> bool:
    return not (Path(data_dir) / "cifar-10-batches-py").exists()
...
def get_cifar10_loaders(
    ...
    """
    获取 CIFAR-10 数据加载器（train/validation split，可选 sealed test）
    ...
    if augmentation:
        ...
        train_transform = _cifar10_eval_transform()

    eval_transform = _cifar10_eval_transform()
    download = _cifar10_download_needed(data_dir)
    try:
        probe = datasets.CIFAR10(root=data_dir, train=True, download=False)
        if not probe._check_integrity():
            download = True
    except RuntimeError:
        download = True
    ...
    eval_full_train = datasets.CIFAR10(
        root=data_dir, train=True, download=download, transform=eval_transform
    )
    validation_dataset = Subset(eval_full_train, validation_indices)
...
def cifar10_split_metadata(
    train_loader: DataLoader, validation_loader: DataLoader, split_seed: int
) -> Dict[str, object]:
    """Return a stable hash and sizes for a CIFAR train/validation split."""
    def indices(loader: DataLoader) -> list[int]:
        dataset = loader.dataset
        if not isinstance(dataset, Subset):
            raise ValueError("loader dataset must be a Subset")
