"""GLUE causal-LM training packs for Phase K Level-1 recovery (SST-2 / RTE / QNLI)."""
...
from src.utils.glue_protocol import SUPPORTED_GLUE_TASKS, assert_test_not_in_selection_path, label_to_verbalizer
from src.utils.qwen_glue_eval import apply_qwen_chat_prompt, build_glue_user_message


class GlueCausalLmDataset(Dataset):
    """Chat-templated prompt + verbalizer packs; labels mask the prompt with -100."""
    ...
    def __init__(
        ...
        *,
        ...
        for index in range(n):
            ...
            answer = label_to_verbalizer(int(row["label"]), task=self.task)
            ...
            # Prefer keeping the answer tokens under max_seq_len.
            ...
            if len(input_ids) > self.max_seq_len:
                # Truncate from the left of the prompt region only.
            ...
            if not any(label != -100 for label in labels):
                raise ValueError("GLUE pack has no supervised tokens; increase max_seq_len")
