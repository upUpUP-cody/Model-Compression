#!/usr/bin/env python3
"""BBH1024 batch=16 VRAM probe (limit_override=32). Does not modify E1/E2 checkpoints.

Writes /mnt/data2/results/bbh_maxtok1024_calib/bbh1024_batch16_probe.json
...
def main() -> None:
    ...
    gk["do_sample"] = False
    dim["gen_kwargs"] = gk
    run.setdefault("hardware", {})["batch_size"] = 16
    run["hardware"]["device"] = "cuda:0"

    payload: Dict[str, Any] = {
        "experiment_id": "BBH1024_BATCH16_PROBE",
        "batch_size": 16,
        "limit_override": 32,
        "max_gen_toks": 1024,
        "do_sample": False,
    ...
    print(
        f"[INFO] BBH1024 batch=16 probe limit_override=32 model={model_path}",
        flush=True,
    )
    ...
    try:
        ...
        payload["oom"] = True
        payload["wall_sec"] = time.time() - t0
        payload["note"] = "torch.cuda.OutOfMemoryError during batch=16 probe"
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    out = OUT_DIR / "bbh1024_batch16_probe.json"
    write_json(out, payload)
    print(
        f"[INFO] batch16_probe oom={payload['oom']} peak={payload['gpu_mem_gb_peak']} "
