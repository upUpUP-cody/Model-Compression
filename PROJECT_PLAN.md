...
- **任务**: 下载并预处理基准数据集
- **数据集选择** (按难度递增):
  1. **MNIST/CIFAR-10**: 简单图像分类 (用于初期验证)
  2. **SQuAD 2.0**: 问答任务 (论文主要基准)
  3. **MMLU**: 多任务语言理解 (可选)
- **产出**: 
...
- **模型选择**:
  - **简单**: 3-5 层 MLP (用于 MNIST)
  - **中等**: ResNet-18/50 (用于 CIFAR-10)
  - **复杂**: BERT-base 或 Qwen-0.5B (用于 SQuAD)
...
### 技术难点预警
1. **Frontier 计算**: 需要大量候选评估,计算成本高
2. **LoRA 恢复**: 超参数敏感,需要细致调参
3. **控制器设计**: 决策逻辑复杂,需多次迭代
4. **大模型实验**: GPU 内存需求高 (建议 >= 24GB VRAM)
